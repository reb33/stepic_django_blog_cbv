...
class Comment(MPTTModel):
    ...
    author = models.ForeignKey(
        User, verbose_name="Автор комментария", on_delete=models.CASCADE, related_name="comments_author"
    )
    content = models.TextField(verbose_name="Текст комментария", max_length=3000)
    time_create = models.DateTimeField(verbose_name="Время добавления", auto_now_add=True)
    time_update = models.DateTimeField(verbose_name="Время обновления", auto_now=True)
    status = models.CharField(
        choices=STATUS_OPTIONS, default="published", verbose_name="Статус комментария", max_length=10
    )
    parent = TreeForeignKey(
        "self",
...
class Rating(models.Model):
    ...
    post = models.ForeignKey(to=Post, on_delete=models.CASCADE, verbose_name="Запись", related_name="ratings")
